  ...
  module chanel_C
    ...
    character(len=f_len) ::  &
      PM6_input,       & !  The name of a PM6_D3H4 arc file
      set,             & !
...
!
  ...
  module sets_of_names_C
    integer, parameter :: ilen=10000, f_len=1000
    character (LEN=f_len) :: &
      data_set_name(-20:ilen),          & !
      all_clean_data_set_names(ilen),   & !  All individual data-set names with all non-alphanumerics set to "_"
      hof_PM7_all(ilen),                & !
      hof_PM6_all(ilen),                & !
      all_solids(ilen)                    !
