...
subroutine coscl1 (a, id, n)
   ...
   !
    ...
    integer :: i, indi, indk, j, k, ikk, iik
    ...
    do k = 1, n
      ...
      summe = 0.d0
      do j = 1, k - 1
        summe = summe + g(indk + j)**2
      end do
      ikk = indk + k
      if (a(ikk) - summe < 0.2d0) then
        g(ikk) = 8.d0
      else
        g(ikk) = sqrt(a(ikk) - summe)
      end if
      do i = k + 1, n
        indi = id(i)
        summe = 0.d0
        do j = 1, k - 1
          summe = summe + g(indi + j)*g(indk + j)
        end do
        iik = indi + k
        g(iik) = (a(iik) - summe)/g(ikk)
      end do
      continue
    end do
    do k = 1, n
